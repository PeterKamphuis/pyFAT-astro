...
def read_config_file(filename,input_parameters):
    ...
    for tmp in tmpfile.readlines():
        ...
        # python is really annoying with needing endlines. Let's strip them here and add them when writing
            Configuration[tmp.split('=', 1)[0].strip().upper()] = tmp.split('=', 1)[1].strip()
    ...
    if input_parameters.installation_check:
        Configuration['CATALOGUE'] = original_dir+'/Installation_Check/FAT_Input_Catalogue.txt'
        Configuration['MAINDIR'] = original_dir+'/Installation_Check/'
        Configuration['OUTPUTCATALOGUE'] =original_dir+'/Installation_Check/Output_N2903.txt'
    ...
    required_configuration_keys = ['STARTGALAXY', 'ENDGALAXY', 'ALLNEW', 'FINISHAFTER', 'CATALOGUE', 'MAINDIR', 'OUTPUTCATALOGUE', 'OUTPUTLOG', 'NEW_OUTPUT', 'OPT_PIXELBEAM', 'VELOCITY_RESOLUTION', 'MAPS_OUTPUT']
